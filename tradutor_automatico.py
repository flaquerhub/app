...
# -*- coding: utf-8 -*-
"""
Script Automatizador de Tradução de Comentários Patrísticos
...
def listar_arquivos_pendentes(diretorio_output, diretorio_traducoes):
    """Lista arquivos Lucas_X.txt que ainda não foram traduzidos"""
    arquivos_lucas = sorted([
        f for f in os.listdir(diretorio_output)
        if f.startswith('Lucas_') and f.endswith('.txt')
    ], key=lambda x: int(x.replace('Lucas_', '').replace('.txt', '')))
    ...
    if os.path.exists(diretorio_traducoes):
        traducoes_existentes = set([
            ...
            if f.startswith('bloco_') and f.endswith('_traduzido.json')
        ])

    pendentes = []
    for i, arquivo in enumerate(arquivos_lucas, start=1):
        nome_traducao = f"bloco_{i:03d}_traduzido.json"
...
def apresentar_arquivo_para_traducao(numero, arquivo_txt, prompt, exemplo_json, conteudo_arquivo):
    ...
    print("\n" + "="*80)
    print(f"ARQUIVO PARA TRADUÇÃO: {numero}/91")
    print("="*80)

    print(f"\nArquivo origem: output/{arquivo_txt}")
    print(f"Arquivo destino: traducoes/bloco_{numero:03d}_traduzido.json")
...
def main():
    ...
    if not pendentes:
        print("✓ Todos os 91 arquivos de Lucas já foram traduzidos!")
        sys.exit(0)

    print(f"\nArquivos pendentes: {len(pendentes)}/91")
    print(f"Arquivos já traduzidos: {91 - len(pendentes)}/91")
    print()
    ...
    print(f"📊 Progresso: {numero}/91 ({numero/91*100:.1f}%)")
    print()
